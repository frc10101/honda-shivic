...
#define CONTROLLER_DRIVEFWD       (tm1600.GetStickZ() + tm1600b.GetStickZ())
//tm1600.GetStickX() + tm1600b.GetStickX() +
#define CONTROLLER_DRIVELR         (tm1600.GetStickY() + tm1600b.GetStickY())  
...
#define CONTROLLER_CARGOSPIN_FWD      tm1600.GetBtn(6) || tm1600b.GetBtn(6)
#define CONTROLLER_CARGOSPIN_BKW      tm1600.GetBtn(9) || tm1600b.GetBtn(9)
...
#define CONTROLLER_CARGOSPIN_FWD      tm1600.GetBtn(3)
#define CONTROLLER_CARGOSPIN_BKW      tm1600.GetBtn(4)
...
#define TUNE_THE_PID 1
  ...
  void TeleopPeriodic() override 
  {
    ...
    float specialSpeedMultiplier = 1.0; //for ball pickup/launcher 
    ...
    if(shiftUp || shiftDown)
    {
      ...
      specialSpeedMultiplier = speeds[speedSelect];

      table->PutNumber("Gear", speedSelect);
      table->PutNumber("MotorMult", specialSpeedMultiplier);
    ...
    if( semiautostate == DO_NOTHING )
    {
      switch(DriveMode)  //since buttons could be shared between hatch vs cargo mode...
      {
        ...
        case DriveMode_Cargo:
          ...
          {
            CargoMotor1.Set(specialSpeedMultiplier);  //3
            CargoMotor2.Set(specialSpeedMultiplier);  //5
          }
          else
          if(CONTROLLER_CARGOSPIN_BKW)
          {

            CargoMotor1.Set(-1.0 * specialSpeedMultiplier);  //1
            CargoMotor2.Set(1.0 * specialpeedMultiplier); //-1
          }  
          else
          {
            CargoMotor1.Set(0.0);
